 ...
 * @LastEditors: BRMBC
 * @LastEditTime: 2024-04-24 13:17:02
 * @FilePath: /RMvision/Armor/armorClassifier.cpp
...
vector<vector<MatrixXd>> Classifier::max_pool(const vector<vector<MatrixXd>> &input, int size)
{
    ...
    {
        vector<MatrixXd> sub;
        for (size_t channel = 0; channel < input[0].size(); channel++)
        {
...
vector<vector<MatrixXd>> Classifier::apply_bias(const vector<vector<MatrixXd>> &input, const vector<double> &bias)
{
    ...
    vector<vector<MatrixXd>> result;
    for (size_t samples = 0; samples < input.size(); samples++)
    {
        vector<MatrixXd> sub;
        for (size_t channels = 0; channels < input[0].size(); channels++)
        {
...
vector<vector<MatrixXd>> Classifier::conv2(const vector<vector<MatrixXd>> &filter, const vector<vector<MatrixXd>> &input)
{
    ...
    {
        vector<MatrixXd> sub;
        for (size_t row = 0; row < filter[0].size(); row++)
        {
